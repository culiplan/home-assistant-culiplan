...
// lovelace/cards/pantry-tracker.ts
...
var ICON_BOX = x`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>`;
var ICON_MINUS = x`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/></svg>`;
var ICON_PLUS = x`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>`;
var ICON_CART = x`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>`;
var ICON_X = x`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>`;
var ICON_ALERT = x`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>`;
var ICON_TRENDING_DOWN = x`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 18 13.5 8.5 8.5 13.5 1 6"/><polyline points="17 18 23 18 23 12"/></svg>`;
function getExpiryStatus(expiresAt) {
  ...
  if (ms <= 7 * MS_PER_DAY) {
    const days2 = Math.ceil(ms / MS_PER_DAY);
    return { label: `${days2}d`, severity: "warning", msRemaining: ms };
  }
...
var FlavorplanPantryTracker = class extends s4 {
  constructor() {
    ...
    __publicField(this, "_activeFilter", "all");
    __publicField(this, "_selectedItemId", null);
  }
  ...
  _getFilteredItems() {
    ...
    return filtered.slice().sort((a3, b2) => {
      const aMsR = getExpiryStatus(a3.expiresAt).msRemaining;
      const bMsR = getExpiryStatus(b2.expiresAt).msRemaining;
  ...
  _getSummary() {
    const all = this._getAllItems();
    const expiring = all.filter((i3) => {
      const { severity } = getExpiryStatus(i3.expiresAt);
      return severity === "critical" || severity === "warning";
    });
  ...
  _renderFilterChips(summary) {
    ...
    return x`
      ...
      (chip) => x`
            ...
            >
              ${chip.label}
              ${chip.count > 0 ? x`<span class="chip-count">${chip.count}</span>` : ""}
            </button>
          `
    )}
      </div>
  ...
  _renderTile(item) {
    ...
    return x`
      <article
        ...
        @click=${(e4) => this._toggleActionSheet(item.id, e4)}
        role="button"
        tabindex="0"
        @keydown=${(e4) => e4.key === "Enter" && this._toggleActionSheet(item.id, e4)}
        ...
        <div class="tile-main">
          <div class="tile-name-row">
            ...
            <div class="tile-badges">
              ${expiry.severity === "critical" || expiry.severity === "warning" ? x`<span class="badge badge--expiry badge--${expiry.severity}">
                    <span class="badge-icon">${ICON_ALERT}</span>
                    ${expiry.label}
                  </span>` : ""}
              ${low ? x`<span class="badge badge--low-stock">
          ...
          <div class="tile-quantity">
            ...
            ${item.unit ? x`<span class="quantity-unit">${item.unit}</span>` : ""}
          </div>
        </div>
        ...
        ${isOpen ? x`
              ...
              >
                <button
                  ...
                  @click=${(e4) => this._decrementItem(item.id, e4)}
                ...
                <button
                  ...
                  @click=${(e4) => this._addToShopping(item, e4)}
      ...
      return x`
        <ha-card>
          <div class="card-content loading">
            ...
            <div class="tile-grid">
              ${[1, 2, 3, 4, 5, 6].map(() => x`<div class="skeleton-tile"></div>`)}
            </div>
          </div>
        </ha-card>
      `;
      ...
      <ha-card>
        <div class="card-header">
          ...
          <div class="header-summary">
            ${summary.expiring > 0 ? x`<span class="summary-badge summary-badge--warning" aria-label="${summary.expiring} items expiring soon">
                  ${summary.expiring}
                </span>` : ""}
            ${summary.lowStock > 0 ? x`<span class="summary-badge summary-badge--low" aria-label="${summary.lowStock} items low in stock">
                  ${summary.lowStock}
                </span>` : ""}
          </div>
        </div>
        <div class="card-content">
          ...
          ${items.length > 0 ? x`<div class="tile-grid">${items.map((i3) => this._renderTile(i3))}</div>` : this._renderEmptyState()}
        </div>
      </ha-card>
    `;
  }
